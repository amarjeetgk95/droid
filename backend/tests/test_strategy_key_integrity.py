...
def test_mean_reversion_strategy_with_flat_volatility_keys():
    ...
    Fail-closed contract: BB touch alone is not enough — a rejection wick, a
    close back inside the band and second-candle confirmation are required.
    """
    ...
    indicators = {
        "volatility": {
            ...
            "bollinger_middle": 25050.0,
            "bollinger_lower": 24990.0,
        },
        "momentum": {
    ...
    candles = [
        ...
        {"open": 24990.0, "high": 25000.0, "low": 24970.0, "close": 24978.0, "volume": 1000},  # exhaustion bar
        {"open": 24980.0, "high": 24995.0, "low": 24960.0, "close": 24995.0, "volume": 1000},  # rejection close
    ]

    # Spot near lower band
    ctx = StrategyContext(
        underlying="NIFTY",
        spot_price=Decimal("24995"),  # Near lower band 24990
        timeframe="5M",
...
def test_mean_reversion_fails_closed_without_candles():
    """BB + RSI extremes with no PIT candles must return None (no intrabar fabrication)."""
    strat = MeanReversionStrategy()
    ctx = StrategyContext(
        underlying="NIFTY",
...
def test_orb_strategy_calculates_opening_range_from_candles():
    ...
    Fail-closed contract: the 5M candle must CLOSE beyond the opening range
    high with measured volume >= 1.3x.
    """
    strat = OpeningRangeBreakoutStrategy()

    candles = [
        {"high": 24820.0, "low": 24780.0, "close": 24810.0, "open": 24790.0, "volume": 1000},
        {"high": 24830.0, "low": 24790.0, "close": 24825.0, "open": 24810.0, "volume": 1000},
        {"high": 24840.0, "low": 24800.0, "close": 24835.0, "open": 24825.0, "volume": 1000},
        {"high": 24850.0, "low": 24820.0, "close": 24845.0, "open": 24825.0, "volume": 5000},
    ]

    # Spot breaks above opening range high (24840); the candle closes above it.
    ctx = StrategyContext(
        underlying="NIFTY",
        spot_price=Decimal("24845"),  # Breaks above high
        timeframe="5M",
        indicators={"atr": 25.0, "volume_ratio": 1.5},
...
def test_gamma_squeeze_strategy_derives_oi_change():
    ...
    Fail-closed contract: real futures OI derivation is only accepted together
    with strike-specific OI and IV expansion evidence.
    """
...
def test_gamma_squeeze_fails_closed_without_strike_oi():
    """Real OI change + IV but no strike-specific OI -> None (fail closed)."""
    strat = GammaSqueezeStrategy()
    ctx = StrategyContext(
        underlying="NIFTY",
        spot_price=Decimal("24820"),
        timeframe="5M",
        indicators={"atr": 20.0},
        mtf={"alignment_score": 75.0},
        fno={"pcr": 1.35, "oi_change_pct": 10.0, "max_pain": 24800.0, "atm_iv": 16.0},
        regime="HIGH_VOL",
    )
    assert strat.detect(ctx) is None
