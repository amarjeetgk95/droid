...
def test_resolve_alias():
    ...
    assert resolve_instrument("bitcoin").symbol=="BTC"
    assert resolve_instrument("btc").symbol=="BTC"
    assert resolve_instrument("BTCUSDT").symbol=="BTC"
    assert resolve_instrument("eth").symbol=="ETH"
    assert resolve_instrument("solana").symbol=="SOL"
    assert resolve_instrument("nifty 50").symbol=="NIFTY"
    assert resolve_instrument("abc") is None
    # Outside fixed 7 universe must not resolve
    assert resolve_instrument("RELIANCE") is None
    assert resolve_instrument("MIDCPNIFTY") is None
    assert resolve_instrument("GOLD") is None

def test_search_case_insensitive_whitespace():
    assert search_instruments("BANKNIFTY")[0].symbol=="BANKNIFTY"
    assert search_instruments("banknifty")[0].symbol=="BANKNIFTY"
    assert search_instruments("  banknifty  ")[0].symbol=="BANKNIFTY"
    # Fixed 7 universe: btc resolves to single BTC canonical
    assert len(search_instruments("btc"))==1
    assert search_instruments("btc")[0].symbol=="BTC"
    assert len(search_instruments("xyz"))==0
    assert len(search_instruments("", limit=3))==3
    # Search for disallowed instrument must be empty
    assert len(search_instruments("RELIANCE"))==0

def test_registry_timeframe():
    cfg=get_by_symbol_exact("BANKNIFTY")
    assert "1m" in cfg.supported_timeframes
    assert "4h" in cfg.supported_timeframes
    assert "1D" in cfg.supported_timeframes
    assert cfg.fno_available is True
    cfg2=get_by_symbol_exact("BTC")
    assert cfg2 is not None
    assert cfg2.fno_available is False
    assert "4h" in cfg2.supported_timeframes
    assert "1D" in cfg2.supported_timeframes
...
def test_volume_unavailable():
    ...
    ta=analyze_timeframe(c, "BTC","15m")
...
def test_forecast_probability_validity():
    ...
    fc=forecast_for_timeframe(feat, ta, "15m","BTC","CRYPTO")
...
def test_multi_timeframe_not_average():
    # New 6-TF weights: 1m 0.10, 5m 0.15, 15m 0.20, 1h 0.20, 4h 0.18, 1D 0.17 = 1.0
    # For 4 TFs (1m,5m,15m,1h): bull weighted 0.10+0.15+0.20=0.45, total 0.65 => 69.2
    ...
    # weighted alignment should be 69.2 (bull 0.45 / 0.65), not simple average of scores (72.25) nor old 75.0
    assert align['overall_bias']=='BULLISH'
    assert align['alignment_score']==69.2
