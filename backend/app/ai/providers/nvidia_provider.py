                ...
                try:
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                        try:
                            ...
                            raise ValueError(f"NVIDIA returned non-JSON content: {c[:400]} (json error: {je})")
                    else:
                        raise ValueError(f"NVIDIA returned non-JSON content: {c[:400]}")
            else:
                parsed = content

            if not isinstance(parsed, dict):
                raise ValueError(f"NVIDIA response root is not a JSON object: {type(parsed)}")
