    ...
    def evaluate(
        ...
        if options_chain and hasattr(options_chain, "strikes"):
            for row in options_chain.strikes:
                side = row.call if opt_type == "CE" else row.put
        ...
        # §11: Edge must be favorable: expected move >= implied move
        if iv_edge < 1.0:
            return None
        ...
        score = calculate_setup_score(
            ...
            direction=direction,
            delta=greeks.delta,
            theta_drag_ratio=getattr(selection.all_candidates[0], "theta_drag_ratio", 12.0),
        ...
        tech_reasons = [
            f"Underlying in clear {direction.replace('_', ' ')} trend (MA alignment)",
            f"Expected move (+{expected_move:.1f} pts) exceeds option implied move by {iv_edge:.2f}x",
            f"Trigger at ₹{trigger:.2f}, Invalidation Stop at ₹{spot_stop:.2f}",
        ]
        contract_sym = getattr(contract, "broker_symbol", "") or getattr(contract, "symbol", "")
        options_reasons = [
            f"Low/Normal IV environment: IV Rank {iv_percentile:.1f}% ({regime.iv_regime})",
            f"Contract: {contract_sym} ({selection.selected_strike_type})",
            f"Delta: {greeks.delta:.2f}, Premium Entry: ₹{entry_premium:.2f}, Stop: ₹{stop_premium:.2f}",
        ]
        risk_reasons = [
        ...
        is_regime_blocked = (direction == "LONG_CALL" and regime.regime == "BEAR") or (direction == "LONG_PUT" and regime.regime == "BULL")
