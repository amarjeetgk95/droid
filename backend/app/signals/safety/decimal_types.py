"""
Decimal Value Types & Normalization for Signal & Execution Engine
...
class Price:
    ...
    def __repr__(self) -> str:
        return f"Price({self._v})"

    def __eq__(self, other):
        return self._v == (other._v if isinstance(other, Price) else D(other)) if isinstance(other, (Price, Decimal, int, str)) else False
...
class Quantity:
    ...
    def __str__(self) -> str:
        return format(self._v.normalize() if self._v == self._v.to_integral_value() else self._v, 'f')

    def __repr__(self) -> str:
        return f"Quantity({self._v})"

    def __eq__(self, other):
        return self._v == (other._v if isinstance(other, Quantity) else D(other)) if isinstance(other, (Quantity, Decimal, int, str)) else False
...
class Money:
    ...
    def __str__(self) -> str:
        return f"{self._ccy} {format(self._v, 'f')}"

    def __repr__(self) -> str:
        return f"Money({self._v}, {self._ccy})"
...
class Percentage:
    ...
    def __init__(self, value: DecimalLike):
        if isinstance(value, float):
            raise TypeError("Percentage must not be float")
        self._v = D(value)
...
class TickSize:
    ...
    def __init__(self, value: DecimalLike):
        if isinstance(value, float):
            raise TypeError("TickSize must not be float")
        self._v = D(value)
        if self._v <= D(0):
...
class Notional:
    ...
    def __init__(self, value: DecimalLike):
        if isinstance(value, float):
            raise TypeError("Notional must not be float")
        self._v = D(value)
...
def normalize_exposure(
    ...
    Returns decimal strings for serialization safety.
    """
...
def validate_quantity(
    quantity: DecimalLike,
    min_qty: DecimalLike,
    quantity_step: DecimalLike,
    lot_size: DecimalLike | None = None,
) -> tuple[bool, str | None]:
    """
