...
class ForecastObject(BaseModel):
    symbol: str
    timeframe: str = Field(pattern="^(1m|5m|15m|1h|30m|4h|1D|1W)$")
    generated_at: str
...
async def get_technical(symbol: str, timeframe: str | None = Query(default=None, pattern="^(1m|5m|15m|1h|30m|4h|1D|1W)$")):
    try:
        result = await analyze_instrument(symbol.strip(), requested_timeframe=timeframe)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    tfs = result["timeframes"]
    # Return per-timeframe technical analysis with disclaimer
    return {
        "data": {
...
async def get_historical_similarity(symbol: str, timeframe: str | None = Query(default="15m", pattern="^(1m|5m|15m|1h|30m|4h|1D|1W)$")):
    try:
