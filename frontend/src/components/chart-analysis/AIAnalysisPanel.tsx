...
export function AIAnalysisPanel({ data }: { data: any }) {
  ...
  // Try to fetch AI explanation on mount, but never break quantitative forecast if AI fails (§39)
  useEffect(()=>{
    let cancelled=false;
    const fetchAI = async ()=>{
      try {
        ...
        if (!cancelled) setAiError('AI temporarily unavailable — quantitative forecast remains available.');
      }
    };
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4">
      ...
      <p className="text-sm text-muted-foreground">
        {bias === 'BULLISH' && `Current short-term structure is bullish and the ${Object.entries(mtf.alignment.biases).filter(([,b]:any)=>b==='BULLISH').map(([tf])=>tf).join('/')} forecasts are aligned. The ${mtf.alignment.dominant_timeframe} remains ${mtf.alignment.interpretation.toLowerCase()}`}
      ...
      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
        ...
        <div className="bg-red-50 border border-red-200 rounded p-2">
          ...
          <ul className="list-disc list-inside text-red-700">
            ...
            <li>Invalidation: {fc15?.invalidation_level ?? '—'} (break of this level weakens forecast)</li>
          </ul>
        </div>
      </div>
      ...
      {aiError && <p className="text-xs text-muted-foreground mt-2">{aiError} Quantitative forecast: ↑ {(fc15?.direction.up*100).toFixed(0)}% (15m).</p>}
      <p className="text-xs text-muted-foreground mt-2">AI explains drivers; it does not invent them. Analysis uses: instrument, technical, F&amp;O, prediction, MTF, historical similarity, data quality ({tf15?.data_quality?.slice(0,60)}), freshness {data.freshness}. AI is optional — ML forecast works if AI is unavailable.</p>
      <p className="text-xs text-muted-foreground">Risk: forecast range {fc15?.expected_range.low.toFixed(0)} – {fc15?.expected_range.high.toFixed(0)} not guaranteed to contain future price. Manage position size and invalidation.</p>
    </div>
  );
}
