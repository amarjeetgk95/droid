...
export type ForecastV2Like = {
  ...
  // Tactical-bias cache honesty (SWR): set by the backend when a payload was
  // replayed from cache rather than produced by a live run.
  stale?: unknown;
  cache_age_s?: unknown;
...
/* — Tactical-bias cache honesty + "why" diagnostics — */

/** True only when the backend served this payload from a stale SWR cache. */
export function isStaleForecast(f: ForecastV2Like | null | undefined): boolean {
  return (f as { stale?: unknown } | null | undefined)?.stale === true;
}

/** Cache age in seconds when the backend reported it, else null. */
export function getCacheAgeSeconds(f: ForecastV2Like | null | undefined): number | null {
  const v = (f as { cache_age_s?: unknown } | null | undefined)?.cache_age_s;
  return typeof v === 'number' && Number.isFinite(v) && v >= 0 ? v : null;
}

/** Settle reason (why the window can/can't be scored), trimmed, or null. */
export function getSettleReason(f: ForecastV2Like | null | undefined): string | null {
  const v = (f as { settle_reason?: unknown } | null | undefined)?.settle_reason;
  if (typeof v !== 'string') return null;
  const s = v.trim();
  return s ? s : null;
}

function readDataHealth(f: ForecastV2Like): Record<string, unknown> | null {
  try {
    const ex = (f as { explain?: unknown }).explain;
    if (ex === null || ex === undefined || typeof ex !== 'object') return null;
    const h = (ex as Record<string, unknown>).data_health;
...
/**
 * Human "why" notes for a horizon card: settle reason, data-health gaps
 * (missing/resampled timeframes, ML/options availability) and the backend's
 * limitations[]. Empty array when the backend supplied none (v1 payloads).
 */
export function forecastWhyNotes(f: ForecastV2Like | null | undefined): string[] {
  if (!f) return [];
  const notes: string[] = [];
  const settle = getSettleReason(f);
  if (getSettlementLabel(f) === 'NO' && settle) notes.push(`Settle: ${settle}`);
  const health = readDataHealth(f);
  if (health) {
    const missing = asStringList(health.missing_timeframes, 8);
    if (missing.length) notes.push(`Missing timeframes: ${missing.join(', ')}`);
    const resampled = asStringList(health.resampled_timeframes, 8);
    if (resampled.length) notes.push(`Resampled from 1m: ${resampled.join(', ')}`);
    if (health.ml_available === false) notes.push('ML forecast unavailable');
    if (health.options_available === false) notes.push('Options context unavailable');
    else if (typeof health.options_data_quality === 'string') {
      const q = health.options_data_quality.trim().toUpperCase();
      if (q && q !== 'LIVE') notes.push(`Options data: ${q}`);
    }
  }
  for (const lim of asStringList(f.limitations, 8)) {
    if (!notes.includes(lim)) notes.push(lim);
  }
  return notes;
}

/** Joined tooltip for `forecastWhyNotes`, or undefined when there is nothing to say. */
export function forecastWhyTooltip(f: ForecastV2Like | null | undefined): string | undefined {
  const notes = forecastWhyNotes(f);
